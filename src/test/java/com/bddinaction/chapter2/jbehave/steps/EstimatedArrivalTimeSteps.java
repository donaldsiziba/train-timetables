...
public class EstimatedArrivalTimeSteps {
    ...
    public void givenTheNextTrainIsScheduledToLeave(@Named("departure-time") LocalTime departureTime, String line) {
        Serenity.getCurrentSession().put("uri", String.format("%s://%s:%s/train-timetables/itinerary/arrivaltime/line/%s/to/%s/at/%s", protocol, host, port, line, Serenity.getCurrentSession().get("destination").toString(), departureTime));
    }
